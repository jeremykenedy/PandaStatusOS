...
| 18 | `auto_rebind` | after three consecutive transport failures on a bound printer, the device runs a scan and, if a hit carries the bound serial at a new address, saves it and binds there; it reports through the wire's own `printer.scan` states 4, 5 and 6 (C7). The decision is pure and host-tested; the scan itself finds nothing until a discovery mechanism is documented, so on hardware today the conclusion is always "sn not matched" | off | a printer bound; a discovery mechanism for it to do anything on real hardware |
| 17 | `restart` | a plain restart, named what it is: `POST /api/restart` restarts the device with every setting kept, and a Restart button appears on the System page; the factory's own restart is a socket command named `reset` (C4) | off | nothing |
