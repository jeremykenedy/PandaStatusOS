...
- Flash dumps live outside the repo entirely, at /Users/jeremykenedy/backups/PandaStatus/.
  Not in .Codex/work/. Not in private/.
- Anything committed to docs/ is scrubbed of credentials, MAC, and serial first.
- Confidential values live in private/secrets/ (moved there from .Codex/work/secrets/
  on 2026-09-11 so that no tracked file names this directory; the hook's literal scan
  reads private/secrets/forbidden-strings.txt) and are referenced in docs/ by name only,
  never by value. Example: write `<WIFI_SSID>`, never the SSID itself.

### Rule 3. Never credit Codex.
...
`.Codex/work/` and are never committed.

```
.Codex/work/notes/      running notes, findings, decisions
.Codex/work/scratch/    throwaway scripts, disassembly output, diffs
private/secrets/         Wi-Fi creds, MAC, serial numbers, printer access code, tokens (moved 2026-09-11)
private/analysis/        extracted strings, symbol maps, offset tables (moved 2026-09-11)
```

This file is untracked (excluded in .git/info/exclude) since 2026-09-11: the tree carries
no reference to it or to this directory. The rules it states are public as CONTRIBUTING.md
(the nine standing rules) and docs/PLAN.md (the phases and gates).
...
The pre-commit hook hard fails if any path under `.Codex/work/` or `private/` is
