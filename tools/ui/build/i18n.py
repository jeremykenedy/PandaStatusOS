...
def cmd_collect():
    ...
    for page, path in sources():
        ...
        for m in ELEM.finditer(text):
            ...
            if k:
                val = re.sub(r"\s+", " ", m.group(3)).strip().replace(PRODUCT, "{product}")
...
# The product name. The markup carries it whole for first paint; every table carries the
# {product} token instead, filled in by the page's one accessor, so no translation can
# split or transliterate it and a rename touches this line and the markup only.
PRODUCT = "PandaStatusOS"
...
def cmd_strings():
    ...
    for lang in langs[1:]:
        ...
        literal = sorted(k for k, v in t.items() if PRODUCT in str(v) or "Panda Status" in str(v))
        if literal: problems.append(f"{lang}: the product name is written out in {', '.join(literal[:4])}; tables carry {{product}}")
    ...
    js = ("// generated by tools/ui/build/i18n.py; do not edit\n"
          f"var PS_PRODUCT = {json.dumps(PRODUCT)};\n"
