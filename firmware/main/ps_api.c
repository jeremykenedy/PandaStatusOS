...
/* GET /api/print: what the printer is doing, which the factory's document does not carry.
 *
 * The firmware has held the percentage and the three temperatures since the effects that read a
 * print were built; nothing ever told the page. This is a route of the clone's own rather than a
 * seventh root on the socket, because the socket document is pinned to the factory's six by gate
 * 2 and by the state test, and a progress figure is not worth breaking that for. The page polls
 * it; a percentage does not need to arrive in the same millisecond it changes.
 *
 * percent is -1 until a report has arrived, and each temperature is PS_TEMP_NONE until one has,
 * so the page can tell "not printing" from "printing at 0%". */
int ps_api_print_get(httpd_req_t *req)
{
    cJSON *o = cJSON_CreateObject();
    if (!o) return send_json(req, NULL);
    ps_lock();
    int pct = g_ps.print_percent;
    ...
    cJSON_AddBoolToObject(o, "printing", pct >= 0);
    cJSON *tmp = cJSON_CreateObject();
    if (tmp) {
        cJSON_AddNumberToObject(tmp, "nozzle", noz);
        cJSON_AddNumberToObject(tmp, "bed", bed);
        cJSON_AddNumberToObject(tmp, "chamber", cham);
