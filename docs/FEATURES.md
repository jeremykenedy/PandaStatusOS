...
| 13 | `preview` | the live preview: `POST /api/preview` pins the bar to a chosen state (idle, printing or error), with a progress, temperatures and a print stage if given, for up to ten minutes; the effects that read the print, the layers and the per-stage rows follow the pin; nothing is stored (A13, B3). Each bar state's row on the Lighting page also carries a button that pins its own state for fifteen seconds with nothing to set first, on the same route and the same clock | off | nothing; with the switch off the route answers 302 like any unknown path |
...
| 20 | `static_ip` | a fixed address on the house network instead of whatever DHCP hands out: address, mask, gateway and DNS, kept in a blob of its own beside the config and applied at boot and before every association. Turning it off puts the DHCP client back without a restart (C9) | off | nothing; with the switch off the route answers 302 like any unknown path |
| 19 | `diagnostics` | while the device cannot reach the network or the printer, the bar blinks a colour for the subsystem and a count for the reason instead of what it would otherwise show, in both modes; the codes are in [TROUBLESHOOTING.md](TROUBLESHOOTING.md) (C8) | off | nothing |
| 18 | `auto_rebind` | after three consecutive transport failures on a bound printer, the device runs a scan and, if a hit carries the bound serial at a new address, saves it and binds there; it reports through the wire's own `printer.scan` states 4, 5 and 6 (C7). The decision is pure and host-tested, and since D-048 the scan actually finds printers: they announce themselves over SSDP multicast and the device listens continuously | off | a printer bound |
| 17 | `restart` | a plain restart, named what it is: `POST /api/restart` restarts the device with every setting kept, and a Restart button appears on the Settings page; the factory's own restart is a socket command named `reset` (C4) | off | nothing |
| 16 | `config_io` | the settings as one file: `GET /api/config` exports everything stored except the three passwords (Wi-Fi, hotspot, printer access code) and `POST /api/config` imports the same document, whole or refused, then pushes the six roots to every client (C3) | off | nothing; the file names the network and the printer, so it is the owner's to keep private |
...
not, and sends nothing the factory page would not. Against the clone the Settings page shows
